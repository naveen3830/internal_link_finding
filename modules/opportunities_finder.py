...
def find_unlinked_keywords(soup, keyword, target_url, nlp_model):
    keyword = keyword.strip()
    if not keyword or not nlp_model:
        return []

    keyword_tokens = [token.lower() for token in keyword.split()]
    ...
    for p_tag in paragraphs:
        ...
        paragraph_word_count = len(original_paragraph_text.split())
        if word_count + paragraph_word_count < exclusion_threshold:
            word_count += paragraph_word_count
            continue
            
        doc = nlp_model(original_paragraph_text)
        doc_tokens = [token.text.lower() for token in doc]

        for i in range(len(doc) - num_keyword_tokens + 1):
            if doc_tokens[i : i + num_keyword_tokens] == keyword_tokens:
                ...
                next_token_index = i + num_keyword_tokens
...
def process_single_url_for_all_keywords(url, keyword_url_pairs, session, nlp_model):
    try:
        headers = {
        ...
        for keyword, target_url in keyword_url_pairs:
            if check_existing_links(full_soup, keyword, url, target_url):
                continue
            unlinked_matches = find_unlinked_keywords(clean_soup, keyword, target_url, nlp_model)
            if unlinked_matches:
