...
def link():
    ...
    def detect_url_language(url):
        ...
        teamviewer_lang_map = {
            'it': ['teamviewer.com/it/'],
            'ja': ['teamviewer.com/ja/'],
            'zh': ['teamviewer.cn/', 'teamviewer.com.cn/', '/cn/','/anydesk.com/zhs/solutions/'],
            'es': ['teamviewer.com/latam/']
        }
    ...
    def filter_languages():
        if st.session_state.selected_languages:
            st.session_state.filtered_df = st.session_state.lang_df[
                st.session_state.lang_df['Language'].isin(st.session_state.selected_languages)
            ]
    ...
    if st.session_state.has_extracted and st.session_state.lang_df is not None:
        ...
        selected_languages = st.multiselect(
            ...
            on_change=filter_languages
        )
        
        # Display filtered results
        if st.session_state.filtered_df is not None:
            st.success(f"Found {len(st.session_state.filtered_df)} URLs in selected languages.")
            st.dataframe(st.session_state.filtered_df)
            
            # Download button
            filtered_urls = st.session_state.filtered_df[['source_url']].rename(
