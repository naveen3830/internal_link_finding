...
def get_chrome_driver():
    ...
    try:
        ...
        # If the above fails, try the full path for Linux
        try:
            service = Service('/usr/bin/chromedriver')
            driver = webdriver.Chrome(service=service, options=chrome_options)
        except Exception as e:
            st.error(f"Failed to initialize Chrome driver: {e}")
            st.info("Please make sure Chrome and ChromeDriver are installed in your environment.")
            return None
            ...
            if 'source_url' in df.columns:
                ...
                with st.spinner('Processing URLs...'):
                    ...
                    with concurrent.futures.ThreadPoolExecutor() as executor:
                        futures = [executor.submit(scrape_content, url) for url in df['source_url']]
                        for i, future in enumerate(concurrent.futures.as_completed(futures)):
                            content = future.result()
                            if search_keyword_in_content(content, user_keyword):
                                matching_urls.append(df['source_url'].iloc[i])
